...
| Step | Command / action |
|------|------------------|
| 1. Archive noise | `python u/cleaner.py` (dry-run) then `python u/cleaner.py --apply` |
| 2. Optional viz archive | `python u/cleaner.py --apply --viz` — moves `u/y_visualization.html`, `u/zDefectsDashboard.html` to `archive/cleanup/` |
| 3. Regenerate reports (if needed) | `python u/yVisualizer.py --config y/qoa_web/qoa_web.json` · `python u/zDefects.py` |
...
| 6. Persona drift | If you edited `Docs/test-user-data/` → `python u/sync_personas.py` |

Monthly (optional): `python u/cleaner.py --apply --purge-archive` — deletes `archive/cleanup/` older than 30 days.
...
- With `--viz`: utility HTML in `u/` (regenerate anytime)
