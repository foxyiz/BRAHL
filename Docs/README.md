...
> **Team handoff:** **[Summary.md](../Summary.md)** (suites, commands, qoa_web status). Lifecycle: **[BRAHL.md](./BRAHL.md)**, **[FoXYiZ.md](./FoXYiZ.md)**, **[rules.md](./rules.md)**. Local web app: **[qoa_web](../qoa_web/README.md)** @ http://127.0.0.1:8765
...
**UI actions:** Use only actions listed in `x/xCapa.csv`. Do **not** use `xWaitFor` — verify elements with `xGetText`, `xClick`, or `xType` (they honor `timeout` in `fStart.json`); `xNavigate` waits for the page body.
