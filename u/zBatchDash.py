...
from _paths import KK_ROOT, Z_DIR
...
def parse_parallel_log(log_path: Path) -> BatchJob:
    ...
    if dash_match:
        ...
        if dash_path.is_file():
            dashboard = dash_path.relative_to(KK_ROOT).as_posix()
    elif run_dir and run_dir.is_dir():
        dash_files = sorted(run_dir.glob("*_zDash.html"))
        if dash_files:
            dashboard = dash_files[0].relative_to(KK_ROOT).as_posix()
    ...
    if job.run_dir:
        ...
        if not job.dashboard:
            ...
            if dash_files:
                job.dashboard = dash_files[0].relative_to(KK_ROOT).as_posix()
    return job


def analyze_run_dir(run_dir: Path, label: str = "") -> BatchJob:
    run_dir = run_dir if run_dir.is_absolute() else KK_ROOT / run_dir
...
def build_html(batch_name: str, jobs: list[BatchJob], failures: list[dict[str, str]]) -> str:
    ...
    for job in jobs:
        ...
        log_link = (
            f'<a href="../{escape_html(job.log_path.relative_to(KK_ROOT).as_posix())}" target="_blank">Log</a>'
